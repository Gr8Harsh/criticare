      ...
      <form onSubmit={form.handleSubmit((d) => mutation.mutate(d))} className="space-y-4 pt-2">
        <FormField control={form.control} name="name" render={({ field }) => (
          <FormItem>
            ...
            <FormControl>
              <Input data-testid="input-proc-name" placeholder="e.g. Appendectomy, CT Scan, Blood Test" {...field} />
            </FormControl>
            <FormMessage />
          </FormItem>
        )} />
        <FormField control={form.control} name="description" render={({ field }) => (
          <FormItem>
            ...
            <FormControl>
              <Textarea data-testid="input-proc-description" placeholder="Brief description..." rows={2} {...field} />
            </FormControl>
            <FormMessage />
          </FormItem>
        )} />
        <FormField control={form.control} name="cost" render={({ field }) => (
          <FormItem>
            ...
            <FormControl>
              <Input type="number" min="0" data-testid="input-proc-cost" placeholder="0" {...field} />
            </FormControl>
            <FormMessage />
          </FormItem>
        )} />
...
export default function ProceduresPage() {
  ...
  const { data: catalog, isLoading } = useQuery<any[]>({
    queryKey: ["/api/procedure-catalog"],
  });
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground">Master catalog of hospital procedures and their charges.</p>
        </div>
        {canManage && (
          <Dialog open={addOpen} onOpenChange={setAddOpen}>
            <DialogTrigger asChild>
              <Button className="hover-elevate shadow-lg" data-testid="button-add-catalog">
                <Plus className="w-5 h-5 mr-2" /> Add Procedure
              </Button>
            </DialogTrigger>
            <DialogContent className="sm:max-w-[440px]">
              <DialogHeader>
                <DialogTitle className="font-display">Add Procedure to Catalog</DialogTitle>
              </DialogHeader>
              <AddEditDialog onClose={() => setAddOpen(false)} />
            </DialogContent>
          </Dialog>
        )}
      </div>
                      ...
                      <TableCell className="text-right">
                        <div className="flex justify-end gap-1">
                          <Dialog open={editItem?.id === item.id} onOpenChange={(open) => !open && setEditItem(null)}>
                            ...
                            <DialogContent className="sm:max-w-[440px]">
                              <DialogHeader>
                                <DialogTitle className="font-display">Edit Procedure</DialogTitle>
                              </DialogHeader>
                              {editItem?.id === item.id && (
                                <AddEditDialog item={editItem} onClose={() => setEditItem(null)} />
                              )}
                            </DialogContent>
                          </Dialog>
                          <Button
